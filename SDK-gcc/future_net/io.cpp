...
{
    if (g_result_len > (MAX_LINE_LEN - 10))
        return;
    if (g_result_len > 0)
        g_result_len += sprintf(g_result + g_result_len, "|");
    g_result_len += sprintf(g_result + g_result_len, "%d", edge);
}

void reset_result() {
    memset(g_result, 0, sizeof(char) * MAX_LINE_LEN);
    g_result[0] = 'N';
    g_result[1] = 'A';
    g_result_len = 0;
}
